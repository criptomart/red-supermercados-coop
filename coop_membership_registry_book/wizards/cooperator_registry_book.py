...
class CooperatorRegistryBook(models.TransientModel):
    ...
    def _get_cooperators_data(self, query):
        ...
        partners = self.env['res.partner'].search_read(
            [query],
            [
                'name', 'barcode_base', 'amount_subscription', 'fundraising_partner_type_ids',
                'vat', 'country_id', 'joining_date', 'leaving_date', 'street', 'zip', 'city',
                'state_id', 'invoice_ids'
            ],
            order="barcode_base"
        )
        ...
        for coop in partners:
            sub_req = self.env['account.invoice'].search_read(
                ...
                [
                    'date_invoice', "invoice_line_ids", 'partner_owned_share_id',
                    'fundraising_category_id', 'amount_total_signed', 'state'
                ],
                order="date"
            )
            for req in sub_req:
                self.summary["total_capital_subscriptions"] += req["amount_total_signed"]
                self.summary["amount_per_type"][req['fundraising_category_id'][1]] += req["amount_total_signed"]
                req["quantity"] = 0
    ...
    def print_cooperator_registry_book(self):
        ...
        return self.env.ref('coop_membership_registry_book.action_report_cooperator_registry_book').report_action(None, data=data)
